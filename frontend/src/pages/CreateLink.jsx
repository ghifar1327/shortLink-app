  ...
  return (
        <main className='p-[5%] mt-[3%] px-[25%] flex-1 bg-[#F3F4F5] flex flex-col gap-5'>
          ...
          <form onSubmit={handleSubmit} className='bg-white p-10 rounded-2xl flex flex-col gap-5'>
            <div>
             <Input id={"original_url"} name="original_url" onChange={handleChange} value={form.original_url} placeholder="https://example.com/your-long-url-here" gray>Destination URL <span className='text-red-500'>*</span></Input>
            ...
            <div>
                ...
                <div className='flex items-center border rounded-l border-gray-300 mt-2'>
                 ...
                 <input id="slug" name="slug" onChange={handleChange} value={form.slug}  placeholder="my-custom-slug" className='flex-1 outline-none p-2 px-3'/>
                </div>
            ...
            <div className="flex flex-col p-5 text-primary bg-[#B4C5FF80]/50 rounded-xl border border-primary">
                ...
                <p className="pl-6"><span className="text-black">Your short link will be: </span>{form.slug ? `short.link/${form.slug}` : 'short.link/your-custom-slug'}</p>
            </div>
